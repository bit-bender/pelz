                                        ...
                                        charbuf *der_asn1_msg_out)
{
  ...
  if ((msg_data_in.tag.chars != NULL) && (msg_data_in.tag.len > 0))
  {
    if (memcmp(parsed_test_msg_data.tag.chars,
               msg_data_in.tag.chars,
               msg_data_in.tag.len) != 0)
    {
      pelz_sgx_log(LOG_ERR, "ASN.1 create input/parse output mismatch");
      PELZ_MSG_free(asn1_msg);
      return MSG_TEST_ASN1_CREATE_PARSE_MISMATCH;       
  ...
  if ((msg_data_in.iv.chars != NULL) && (msg_data_in.iv.len > 0))
  {
    if ((memcmp(parsed_test_msg_data.iv.chars,
                msg_data_in.iv.chars,
                msg_data_in.iv.len) != 0))
    {
      pelz_sgx_log(LOG_ERR, "ASN.1 create input/parse output mismatch");
      PELZ_MSG_free(asn1_msg);
      return MSG_TEST_ASN1_CREATE_PARSE_MISMATCH;            
  ...
  if (test_select == ASN1_PARSE_FUNCTIONALITY)
  {
    PELZ_MSG_free(asn1_msg);
    return MSG_TEST_OK;
  }
                                               ...
                                               EVP_PKEY *deconstruct_priv)
{
  ...
  if (((test_select <= MSG_TEST_SELECT_START) ||
       (test_select >= MSG_TEST_SELECT_END)) ||
       (construct_cert == NULL) ||
       (construct_priv == NULL) ||
                                           ...
                                           ReqTestSelect test_select)
{
  ...
  // invoke proper handler for invalid parameter test
  switch (test_select)
  {
  case REQ_TEST_WRAP_NULL_KEY_ID:
  case REQ_TEST_WRAP_EMPTY_KEY_ID:
  ...
  case REQ_TEST_WRAP_UNWRAP:
    ...
    break;
  default:
    // any other test selection is invalid
    pelz_sgx_log(LOG_ERR, "invalid request handling test selection");
    return REQ_TEST_INVALID_TEST_SELECTION;
    break;
  }
                                               ...
                                               charbuf der_req_priv)
{
  ...
  if (((cipher.chars == NULL) || (cipher.len == 0)) ||
      ((key_id.chars == NULL) || (key_id.len == 0)) ||
      ((der_req_cert.chars == NULL) || (der_req_cert.len == 0)) ||
      ((der_req_priv.chars == NULL) || (der_req_priv.len == 0)))
  {
    pelz_sgx_log(LOG_ERR, "invalid input parameter");
    return REQ_TEST_INVALID_TEST_PARAM;
  }

  X509 *requestor_cert = deserialize_cert(der_req_cert);
  EVP_PKEY *requestor_priv = deserialize_pkey(der_req_priv);

  if (X509_check_private_key(requestor_cert, requestor_priv) != 1)
  {
  ...
  const char *status_str = "test status";
  size_t status_str_len = strlen(status_str);

  PELZ_MSG_DATA req_data = { .msg_type = REQUEST,
                             .req_type = KEY_WRAP,
                             .cipher = new_charbuf(cipher.len),
                             .tag = new_charbuf(0),
                             .iv = new_charbuf(0),
                             .key_id = new_charbuf(key_id.len),
                             .data = new_charbuf(pt_str_len),
                             .status = new_charbuf(status_str_len) };

  memcpy(req_data.cipher.chars,
         (const unsigned char *) cipher.chars,
         req_data.cipher.len);
  memcpy(req_data.key_id.chars,
         (const unsigned char *) key_id.chars,
         req_data.key_id.len);
  ...
  // check peer certificate extracted from message against known copy
  if (X509_cmp(deconstructed_responder_cert, pelz_id.cert) != 0)
  {
    X509_free(deconstructed_responder_cert);
    free_charbuf(&deconstructed_response_data.cipher);
    free_charbuf(&deconstructed_response_data.key_id);
